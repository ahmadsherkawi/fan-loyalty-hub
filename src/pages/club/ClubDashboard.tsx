...
export default function ClubDashboard() {
  ...
  useEffect(() => {
    if (isPreview) {
      setClub({ id: "preview", admin_id: "preview", name: "Demo FC", logo_url: null, primary_color: "#16a34a", country: "", city: "", stadium_name: null, season_start: null, season_end: null, status: previewClubStatus, created_at: "", updated_at: "" });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const { count: fans } = await supabase.from("fan_memberships").select("*", { count: "exact", head: true }).eq("club_id", clubData.id);
      const { count: activities } = await supabase.from("activities").select("*", { count: "exact", head: true }).eq("program_id", programId).eq("is_active", true);
      const { count: rewards } = await supabase.from("rewards").select("*", { count: "exact", head: true }).eq("program_id", programId).eq("is_active", true);
      const { count: claims } = await supabase.from("manual_claims").select("*", { count: "exact", head: true }).eq("status", "pending");
      const { data: completions } = await supabase.from("activity_completions").select("points_earned, activities!inner(program_id)").eq("activities.program_id", programId);
      const totalPoints = completions?.reduce((s, c) => s + c.points_earned, 0) ?? 0;
      setStats({ fans: fans ?? 0, activities: activities ?? 0, rewards: rewards ?? 0, claims: claims ?? 0, points: totalPoints });
  ...
  const statItems = [
    ...
    { icon: <Zap className="h-5 w-5" />, label: "Activities", value: stats.activities, color: "from-blue-500/20 to-blue-500/5" },
    { icon: <Gift className="h-5 w-5" />, label: "Rewards", value: stats.rewards, color: "from-accent/20 to-accent/5" },
    { icon: <FileCheck className="h-5 w-5" />, label: "Pending Claims", value: stats.claims, color: "from-orange-500/20 to-orange-500/5" },
    { icon: <Trophy className="h-5 w-5" />, label: "Points Issued", value: stats.points, color: "from-purple-500/20 to-purple-500/5" },
  ];

  const actions = [
    { title: "Manage Activities", desc: "Create and edit fan activities", disabled: !program, onClick: () => navigate("/club/activities"), icon: <Zap className="h-6 w-6 text-primary" /> },
    { title: "Manage Rewards", desc: "Configure redemption rewards", disabled: !program, onClick: () => navigate("/club/rewards"), icon: <Gift className="h-6 w-6 text-accent" /> },
    { title: "Review Claims", desc: "Approve manual submissions", disabled: !program, onClick: () => navigate("/club/claims"), icon: <FileCheck className="h-6 w-6 text-orange-500" /> },
  ];
          ...
          {statItems.map((s) => (
            <Card key={s.label} className="rounded-2xl border-border/50 overflow-hidden">
              <CardContent className="pt-6 text-center relative">
                <div className={`mx-auto mb-3 h-11 w-11 rounded-xl bg-gradient-to-br ${s.color} flex items-center justify-center text-foreground`}>
                  {s.icon}
                </div>
